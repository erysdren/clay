...
typedef struct {
	...
	xcb_window_t window;
	xcb_font_t fonts[1];
} Clay_XCBRendererData;

static inline Clay_Dimensions Clay_XCB_MeasureText(Clay_StringSlice text, Clay_TextElementConfig *config, void *userData)
{
	Clay_XCBRendererData *renderData = (Clay_XCBRendererData *)userData;
	Clay_Dimensions textSize = {0, 0};
	return textSize;
}

static void Clay_XCB_Render(Clay_XCBRendererData *rendererData, Clay_RenderCommandArray *renderCommands)
{
	for (size_t i = 0; i < renderCommands->length; i++)
	{
		Clay_RenderCommand *renderCommand = Clay_RenderCommandArray_Get(renderCommands, i);
		const Clay_BoundingBox bounding_box = renderCommand->boundingBox;
		xcb_rectangle_t rect = {(int)bounding_box.x, (int)bounding_box.y, (int)bounding_box.width, (int)bounding_box.height};
		...
		switch (renderCommand->commandType)
		{
			case CLAY_RENDER_COMMAND_TYPE_RECTANGLE:
			{
				...
				uint32_t color = ((uint8_t)config->backgroundColor.a << 24) | ((uint8_t)config->backgroundColor.r << 16) | ((uint8_t)config->backgroundColor.g << 8) | (uint8_t)config->backgroundColor.b;
				xcb_change_gc(rendererData->connection, rendererData->gcontext, XCB_GC_FOREGROUND, (uint32_t[]){color});
				xcb_poly_fill_rectangle(rendererData->connection, rendererData->window, rendererData->gcontext, 1, &rect);
				break;
			}
			...
			case CLAY_RENDER_COMMAND_TYPE_TEXT:
			{
				...
				uint32_t color = ((uint8_t)config->textColor.a << 24) | ((uint8_t)config->textColor.r << 16) | ((uint8_t)config->textColor.g << 8) | (uint8_t)config->textColor.b;
				xcb_change_gc(rendererData->connection, rendererData->gcontext, XCB_GC_FOREGROUND | XCB_GC_FONT, (uint32_t[]){color, rendererData->fonts[config->fontId]});
				xcb_image_text_8(rendererData->connection, config->stringContents.length, rendererData->window, rendererData->gcontext, rect.x, rect.y, config->stringContents.chars);
				break;
			}
		}
	}
